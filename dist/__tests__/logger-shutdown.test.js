...
describe('production logger shutdown ownership', () => {
    test.each(['before', 'after', 'automatic'])('%s logger initialization', async (mode) => {
        const child = (0, child_process_1.spawn)(process.execPath, ['-e', script], {
        ...
        try {
            ...
            if (mode === 'automatic') {
                expect(messages).not.toContainEqual(expect.objectContaining({ drained: true }));
            }
            else {
